(function() {
  ...
  User.sync({
    ...
    return console.log("create table USERS successfully");
  });
  Comment.sync({
    force: true
  }).on('success', function() {
    return console.log("create table COMMENTSs successfully");
  });
  Article.sync({
    force: true
  }).on('success', function() {
    return console.log("create table ARTICLESS successfully");
  });
  u = User.build({
    name: 'lay zhu',
